...
```python
task(label: str, **attributes: Any) -> _PipelineTaskFactory
```
...
* **`label`**
  (`str`)
  –The label for the dreadnode task(s). This is used to group
  and identify related runs in the Dreadnode UI.
* **`**attributes`**
  (`Any`, default:
  `{}`
  )
  –Additional key-value attributes to attach to the task span.
...
> > > ===============================================
> > >
> > > my\_task = pipeline.as\_task(label="test-prompt").run(scorers=[...])
> > >
> > > Execute the task
> > > ================
> > >
> > > result\_span = await my\_task.run()
> > >
> > > Execute the task 10 times and get the best 3
> > > ============================================
> > >
> > > best\_chats = await my\_task.top\_n(10, 3, as\_outputs=True)
