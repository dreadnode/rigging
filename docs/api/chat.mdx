...
### inject\_tool\_prompt

```python
inject_tool_prompt(
    tools: Sequence[Tool[..., Any]], mode: ToolMode
) -> Chat
```
...
```python
strip(
    model_type: type[Model], fail_on_missing: bool = False
) -> Chat
```
...
clone(
    *,
    ...
    chat: Chat | None = None,
    callbacks: bool
    | Sequence[MapChatCallback | ThenChatCallback] = True,
) -> ChatPipeline
```
...
* **`callbacks`**
  (`bool | Sequence[MapChatCallback | ThenChatCallback]`, default:
  `True`
  )
...
* **`mode`**
  ...
  –The tool calling mode to use (e.g., "xml", "json-in-xml", "api").
* **`choice`**
  (`ApiToolChoice | None`, default:
  `None`
  )
...
* **`add_stop_token`**
  ...
  –When using natively parsed tools ("xml", "json-in-xml"), use stop tokens to
