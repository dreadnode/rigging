...
Decorate any regular Python function (including static methods) with `@rigging.tool.tool` to make it usable by the Rigging framework.
...
If your tool logic resides within a class and needs access to instance state (`self`), use the `@rigging.tool.tool_method` decorator instead.
...
async with rg.tool.mcp("stdio", command=command, args=args) as mcp_client:
...
async with rg.tool.mcp("sse", url=MCP_SSE_URL) as mcp_client:
    # mcp_client.tools contains the list of tools discovered from the server
...
try:
    ...
    robopages_tools = rg.tool.robopages(ROBOPAGES_URL)
