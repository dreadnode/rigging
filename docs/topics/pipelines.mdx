...
## Watch Callbacks (Passive Monitoring)
...
## Handling Parsing Requirements (`until_parsed_as`)
...
## Handling Failures (`on_failed`, `catch`)

Pipelines provide robust ways to handle errors during generation or callback execution.

### `on_failed` Mode

The `on_failed` parameter (set via `.catch()` or on `run_many`/`run_batch`/`run_over`) determines behavior when a catchable error occurs:
...
- **`"include"`:** The chat is marked with `.failed = True` and the exception stored in `.error`. The chat is included in the results.
...
### Defining Catchable Errors (`catch`)
